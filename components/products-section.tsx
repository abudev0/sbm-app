...
/** Final narx + old narxni izchil hisoblab beramiz (percent-based chegirma bo'lsa) */
function computePrice(basePrice?: number, discountPercent?: number, oldPriceInput?: number) {
  ...
  if (!base) {
    return { final: 0, old: undefined as number | undefined, percent: 0 }
  }
  const hasDiscount = d > 0
  const final = hasDiscount ? Math.round(base * (1 - d / 100)) : base
  // Old price: agar chegirma bo'lsa base ni ko'rsatamiz, bo'lmasa kelgan oldPriceInput bo'lsa ko'rsatamiz
  ...
  return (
    <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div ref={viewportRef} className="relative overflow-hidden outline-none" tabIndex={0} aria-label={t("title")}>
        <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-0 w-8 sm:w-12 to-transparent" />
        <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 right-0 w-8 sm:w-12 to-transparent" />
        ...
        <div className="flex items-center gap-0 sm:gap-2">
          ...
          >
            {products.map((p) => {
              ...
              // --- Slug fallback (asosiy farq shu) ---
              const slug = p.slug || String(p.id) // grid sahifadagi kabi

              // --- Rasm ---
              const img = pickImage(p)

              // --- Narx/chegirma (izchil hisob) ---
              ...
              return (
                ...
                >
                  <ProductCard
                    ...
                    oldPrice={old}                // old price (agar bo‘lsa)
